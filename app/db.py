...
class Database:
    ...
    async def init_tables(self):
        ...
        # Legacy prediction profiles were keyed by Telegram ID. Move them to
        # the canonical account ID so email-only users can use predictions too.
        ...
        if "telegram_id" in profile_columns and "user_id" not in profile_columns:
            await self.conn.execute("DROP TABLE IF EXISTS prediction_profiles_by_user")
            await self.conn.execute(
                """
                CREATE TABLE prediction_profiles_by_user (
                    user_id INTEGER PRIMARY KEY,
                    display_name TEXT NOT NULL,
                    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            ...
            await self.conn.execute(
                """
                INSERT OR IGNORE INTO prediction_profiles_by_user(
            ...
            await self.conn.execute(
                "ALTER TABLE prediction_profiles_by_user RENAME TO prediction_profiles"
            )
        await self.conn.execute(
            """
